...
void loop(){
    ...
    // Re-set input config before each thermocouple read
    ads1118.setInputSelected(ads1118.DIFF_0_1);
    ads1118.setFullScaleRange(ads1118.FSR_0256);

    float millivolts = ads1118.getMilliVolts();
    //Serial.println(String(millivolts, 6) + " mV");

    // Cold junction compensation (internal sensor gives ambient in °C)
    float coldJunction = ads1118.getTemperature();  

    // K-type thermocouple: ~0.041 mV/°C sensitivity
    float hotJunction = (millivolts / 0.041) + coldJunction;
    Serial.println(String(hotJunction, 2) + " C");
